...
1. `https://freevirtualserialports.com/` 에서 무료 버전을 다운로드해 설치합니다.
   (관리자 권한이 필요할 수 있습니다.)
2. 설치 후 프로그램을 실행하면 가상 포트 페어를 관리하는 화면이 나옵니다.
   보통 **"Add pair"** (또는 이와 유사한 이름의 버튼/메뉴)를 클릭하면 새 포트 페어를 만들 수 있습니다.
3. 두 포트 이름을 지정합니다. 이 README에서는 `COM10`(GUI용), `COM11`(시뮬레이터용)로
   가정하고 설명합니다. 다른 이름을 쓰셔도 되며, 그 경우 아래 실행 단계에서 그 이름으로 바꿔 입력하면 됩니다.
4. 생성 버튼을 눌러 페어를 만들면, Windows 장치 관리자의 "포트(COM & LPT)" 항목에
   두 포트가 나타납니다. (버전에 따라 메뉴 명칭/위치가 조금 다를 수 있으니, 설치된
   프로그램의 도움말이나 "Pair 추가"류 메뉴를 참고하세요.)
5. 무료 버전은 **같은 PC 안에서 포트 페어를 로컬로 연결하는 기능까지는 무료**이고,
   원격(다른 PC 간) 연결 등 고급 기능은 유료입니다. 이 실습에서는 로컬 페어만 있으면 충분합니다.

### com0com을 대신 쓰고 싶다면 (참고용, 비권장)

com0com은 무료·오픈소스이고 여전히 동작하는 환경도 있지만, 커널 드라이버 특성상
설치가 안 되거나 코드 52 오류가 나는 사례가 보고됩니다. 그래도 시도해보고 싶다면
SourceForge 공식 프로젝트(`sourceforge.net/projects/com0com/` 또는
`sourceforge.net/projects/signed-drivers/files/com0com/`)에서만 받으시고,
문제가 생기면 미련 없이 위의 Free Virtual Serial Ports로 넘어가는 걸 추천합니다.
(com0com을 검색하면 나오는 일부 제3자 사이트에서 "서명된 최신 빌드"를 광고하는데,
공식 배포처가 아니므로 드라이버 종류의 소프트웨어는 다운로드하지 않는 게 안전합니다.)

## 3. 프로젝트 열기 및 빌드

1. `SerialTurboPumpDemo.sln`을 Visual Studio 2022로 엽니다.
2. 솔루션 탐색기에서 두 프로젝트(`SerialGuiApp`, `DeviceSimulator`)가 보이는지 확인합니다.
3. `빌드 > 솔루션 빌드` (Ctrl+Shift+B) 로 둘 다 빌드되는지 확인합니다.
   - `System.IO.Ports` NuGet 패키지가 자동 복원됩니다. 인터넷 연결이 필요합니다.

## 4. 실행 순서

1. **DeviceSimulator 먼저 실행**
   - `DeviceSimulator` 프로젝트를 우클릭 → 디버그 > 새 인스턴스 시작
   - 실행되면 포트 이름을 물어봅니다. 페어 중 하나(예: `COM11`)를 입력합니다.
   - "[대기 중] COM11 @ 9600bps 에서 명령을 기다리는 중..." 메시지가 뜨면 정상입니다.
   - 이후 2초마다 자동으로 `TELEMETRY:...` 메시지를 콘솔에 출력하며 자체 송신합니다.

2. **SerialGuiApp 실행**
   - `SerialGuiApp`을 시작 프로젝트로 설정하고 F5로 실행합니다.
   - 상단에서 포트로 나머지 하나(예: `COM10`)를 선택하고 **연결** 클릭.
   - 연결 성공 시 상태 표시등이 초록색으로 바뀌고, 텔레메트리 값(온도/압력/밸브 상태)이
     2초 주기로 자동 갱신되는 걸 볼 수 있습니다 (DeviceSimulator가 스스로 보내는 값).
...
모두 줄바꿈(`\n`)으로 구분되는 단순 텍스트 프로토콜입니다. 실무에서는 이 자리에
...
- `MainWindow.xaml.cs`의 `SerialPort.DataReceived` 핸들러: 백그라운드 스레드에서
  발생하므로, UI 요소를 직접 건드리지 않고 `Dispatcher.Invoke`로 넘겨서 갱신합니다.
  이 부분이 WPF/MFC 계측 제어 GUI에서 가장 흔히 실수하는 지점(크로스 스레드 예외)입니다.
- 수신 데이터가 한 번에 완전한 줄 단위로 오지 않을 수 있으므로, `StringBuilder` 버퍼에
  누적했다가 개행 문자를 기준으로 잘라 처리합니다. 실제 계측 장비와 통신할 때도
  이 패턴이 거의 그대로 필요합니다.
- `DeviceSimulator`는 `System.Threading.Timer`로 실제 장비처럼 비동기 텔레메트리를
  스스로 밀어 넣습니다 — 요청 없이도 상태가 계속 흘러 들어오는 상황을 만들어,
  Telemetry(비동기 보고)와 Telecommand(요청-응답)의 차이를 체감할 수 있게 했습니다.
...
- MFC로 동일한 기능을 다시 구현해보며 WPF와의 차이 비교
